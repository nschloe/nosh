...
//       psiLexicographic->replaceMap( standardMap );
  ...
  bifurcationList.set("Bifurcation Parameter","dummy");
  bifurcationList.set("Type","Pitchfork");
  bifurcationList.set("Formulation","Moore-Spence"); // default

  int NumGlobalUnknowns = psiLexicographic->globalLength();
  int numRealUnknowns = 2*NumGlobalUnknowns+1;
  Teuchos::RCP<Epetra_Map> map = Teuchos::rcp( new Epetra_Map(numRealUnknowns,
                                                              0,
                                                              *eComm) );
  Teuchos::RCP<Epetra_Vector> dummyVec = Teuchos::rcp( new Epetra_Vector(*map) );

  Teuchos::RCP<NOX::Epetra::Vector> normalizationVector =
                                   Teuchos::rcp( new NOX::Epetra::Vector(dummyVec) );
  normalizationVector->init(1.0);
  bifurcationList.set("Length Normalization Vector", normalizationVector );

  Teuchos::RCP<NOX::Epetra::Vector> initialNullVector =
                                   Teuchos::rcp( new NOX::Epetra::Vector(dummyVec) );
  initialNullVector->init(0.0);
  bifurcationList.set("Initial Null Vector", initialNullVector );

  Teuchos::RCP<NOX::Epetra::Vector> asymVector =
                                   Teuchos::rcp( new NOX::Epetra::Vector(dummyVec) );
  asymVector->init(1.0);
  bifurcationList.set("Asymmetric Vector", asymVector );
  // ---------------------------------------------------------------------------
