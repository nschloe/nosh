...
def get_operator_core_edge_code_from_integral(
    ...
    return _get_operator_core_edge_code(
...
def _get_operator_core_edge_code(
        namespace, class_name,
    ...
    members_init.append('nosh::operator_core_edge(%s)' % parent_init)
    ...
    with open(os.path.join(templates_dir, 'operator_core_edge.tpl'), 'r') as f:
        src = Template(f.read())
        code = src.substitute({
            'name': class_name,
