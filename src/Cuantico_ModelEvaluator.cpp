...
typedef void (Cuantico::ModelEvaluator::*dfdpGetter)(const Epetra_Vector &x,
...
ModelEvaluator (
  ...
  evalModelTime_( Teuchos::TimeMonitor::getNewTimer(
                    "Cuantico: ModelEvaluator::evalModel" ) ),
  computeFTime_( Teuchos::TimeMonitor::getNewTimer(
                   "Cuantico: ModelEvaluator::evalModel:compute F" ) ),
  computedFdpTime_( Teuchos::TimeMonitor::getNewTimer(
                      "Cuantico: ModelEvaluator::evalModel:compute dF/dp" ) ),
  fillJacobianTime_( Teuchos::TimeMonitor::getNewTimer(
                       "Cuantico: ModelEvaluator::evalModel:fill Jacobian" ) ),
  fillPreconditionerTime_( Teuchos::TimeMonitor::getNewTimer(
                             "Cuantico: ModelEvaluator::evalModel::fill preconditioner" ) ),
#endif
...
#endif
    const Teuchos::RCP<Cuantico::JacobianOperator> & jac =
      Teuchos::rcp_dynamic_cast<Cuantico::JacobianOperator>(W_out, true);
...
{
  return Teuchos::rcp(new Cuantico::State(x, mesh_));
}
// =============================================================================
Teuchos::RCP<const Epetra_Vector>
ModelEvaluator::
