// $Id: LOCA_Stepper.C,v 1.66 2008/08/05 21:01:18 etphipp Exp $
// $Source: /space/CVS/Trilinos/packages/nox/src-loca/src/LOCA_Stepper.C,v $
...
LOCA::Stepper::start() {
  ...
  // Compute eigenvalues/eigenvectors and recreate the eigenstepper if requested
  if (calcEigenvalues) {
...
LOCA::Stepper::postprocess(LOCA::Abstract::Iterator::StepStatus stepStatus)
{
  ...
  *curPredictorPtr = curGroupPtr->getPredictorTangent()[0];
  ...
  // Compute eigenvalues/eigenvectors, recreate eigensolver
  if (calcEigenvalues) {
    Teuchos::RCP< std::vector<double> > evals_r;
    Teuchos::RCP< std::vector<double> > evals_i;
    Teuchos::RCP< NOX::Abstract::MultiVector > evecs_r;
